...
static int LuaAddCommand(lua_State* L)
{
	...
	const MerryCommand* command = g_commands->AddCommand(commandName, funcRef, triggerKey);
	if (!command)
	{
...
{
	new MerryInformationDialog(wxT("Message"), wxString(lua_tostring(L, 1), wxConvLocal));
	return 0;
}
